...
def get_model(params, categorical = None):
    if categorical is None:
        categorical = ['seller_zip_code_prefix', 'customer_zip_code_prefix']  # TODO: get from config
    model = CatBoostRegressor(cat_features=categorical, verbose=0, train_dir="/srv/data/catboost_info", **params)

    return model

def apply_model(config, model_path, data):
    categorical = ['seller_zip_code_prefix', 'customer_zip_code_prefix']
    params = config['catboost_params']
    model = get_model(params, categorical)
